...
if ( ! class_exists( 'Kindergarten_Playgroup_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Kindergarten_Playgroup_WPTT_WebFont_Loader {
		 ...
		 * @return void
		 */
		public function counter_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_counterd( 'delete_fonts_folder' ) && ! wp_installing() ) {
					wp_counter_event( time(), self::CLEANUP_FREQUENCY, 'delete_fonts_folder' );
				}
			}
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Kindergarten_Playgroup_WPTT_WebFont_Loader( $url );
