...
if ( ! class_exists( 'WPCpetNotice' ) ) {
	class WPCpetNotice {
		...
		function notice_scripts() {
			wp_enqueue_style( 'wpcpet-notice', esc_url( get_template_directory_uri() . '/assets/css/notice.css' ) );
		}
		...
		function admin_notice() {
			...
			if ( ! get_user_meta( $user_id, 'wpcpet_notice_ignore', true ) ) {
				add_thickbox();
				?>
              <div class="wpcpet-notice notice">
                <div class="wpcpet-notice-thumbnail">
                  <a href="https://wordpress.org/themes/wpcpet/" target="_blank">
                    <img src="<?php echo esc_url( get_template_directory_uri() . '/screenshot.png' ); ?>"
                         alt="WPCpet" />
                  </a>
                </div>
                <div class="wpcpet-notice-text">
                  <h3>Warmly welcome to WPClever’s WPCpet theme</h3>
                  <p>
                  ...
                  <ul class="wpcpet-notice-ul">
                    ...
                    <li>
						...
						if ( function_exists( 'wc_get_current_admin_url' ) ) {
							$ignore_url = add_query_arg( 'wpcpet_notice_ignore', '1', wc_get_current_admin_url() );
						} else {
							$ignore_url = admin_url( '?wpcpet_notice_ignore=1' );
						}
						?>
		...
		function notice_ignore() {
			...
			if ( isset( $_GET['wpcpet_notice_ignore'] ) ) {
				if ( $_GET['wpcpet_notice_ignore'] == '1' ) {
					update_user_meta( $user_id, 'wpcpet_notice_ignore', 'true' );
				} else {
					delete_user_meta( $user_id, 'wpcpet_notice_ignore' );
				}
			}
		}
	}
